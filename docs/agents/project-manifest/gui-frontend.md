...
- `api.repositories` — `list()`, `get(id)`, `create(data)`, `update(id, data)`, `delete(id)`, `touchRefreshTimestamp(id)`, `credentialOptions(id)`, `updateCredential(id, credentialId)`
...
| `updateCredential(id, credentialId)` | `PUT /api/repositories/:id/credential` | Associate a credential with the repository, or clear the association. Pass `''` (empty string) to clear. Returns the updated repository object. |

**`updateCredential` caller contract:** `credentialId` must always be a `string`. Passing `undefined` causes `JSON.stringify` to silently strip the key from the request body, producing an empty `{}` payload instead of `{ credentialId: '' }`. Always pass `''` when the intent is to clear the association.

**`credentialOptions` auto-select semantics:** Each returned option includes an `auto` boolean. The GUI pre-selects an option only when exactly one option is returned with `auto: true`. When multiple options share `auto: true` (multiple credentials for the same host), none is pre-selected — the user must choose explicitly.
